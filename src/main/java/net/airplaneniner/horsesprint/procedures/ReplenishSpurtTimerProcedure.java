...
public class ReplenishSpurtTimerProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		...
		if (!HorseSprintModVariables.MapVariables.get(world).horseIsSprinting) {
			if (HorseSprintModVariables.MapVariables.get(world).horseSpurtTimer < HorseSprintModVariables.MapVariables.get(world).horseMaxSpurt) {
				if (HorseSprintModVariables.MapVariables.get(world).SecondTimer == 20) {
					if ((entity.getVehicle()) instanceof LivingEntity _livingEntity3 && _livingEntity3.getAttributes().hasAttribute(HorseSprintModAttributes.HORSE_SPURT_TIMER.get()))
						_livingEntity3.getAttribute(HorseSprintModAttributes.HORSE_SPURT_TIMER.get())
								.setBaseValue((((entity.getVehicle()) instanceof LivingEntity _livingEntity1 && _livingEntity1.getAttributes().hasAttribute(HorseSprintModAttributes.HORSE_SPURT_TIMER.get())
										? _livingEntity1.getAttribute(HorseSprintModAttributes.HORSE_SPURT_TIMER.get()).getBaseValue()
										: 0) + 0.5));
					HorseSprintMod.LOGGER.info(new java.text.DecimalFormat("##.##").format((entity.getVehicle()) instanceof LivingEntity _livingEntity5 && _livingEntity5.getAttributes().hasAttribute(HorseSprintModAttributes.HORSE_SPURT_TIMER.get())
