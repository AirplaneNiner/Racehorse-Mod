...
public class HorseSprintModAttributes {
	...
	public static void addAttributes(EntityAttributeModificationEvent event) {
		Stream.of(ModEntities.HORSE_GENETIC).filter(DefaultAttributes::hasSupplier).map(entityType -> (EntityType<? extends LivingEntity>) entityType).toList().forEach(entity -> event.add(entity, HORSE_STAMINA.get()));
		Stream.of(ModEntities.HORSE_GENETIC).filter(DefaultAttributes::hasSupplier).map(entityType -> (EntityType<? extends LivingEntity>) entityType).toList().forEach(entity -> event.add(entity, HORSE_SPURT.get()));
		Stream.of(ModEntities.HORSE_GENETIC).filter(DefaultAttributes::hasSupplier).map(entityType -> (EntityType<? extends LivingEntity>) entityType).toList().forEach(entity -> event.add(entity, HORSE_SPURT_TIMER.get()));
		Stream.of(ModEntities.HORSE_GENETIC).filter(DefaultAttributes::hasSupplier).map(entityType -> (EntityType<? extends LivingEntity>) entityType).toList().forEach(entity -> event.add(entity, HORSE_MAX_SPURT_TIMER.get()));
	}
}