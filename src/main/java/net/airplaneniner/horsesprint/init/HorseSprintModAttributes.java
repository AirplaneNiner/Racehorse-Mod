...
public class HorseSprintModAttributes {
    ...
    public static final RegistryObject<Attribute> WEIGHT = REGISTRY.register("weight", () -> new RangedAttribute("attribute.horse_sprint.weight", 400, 0, 1000).setSyncable(true));
	...
	public static void addAttributes(EntityAttributeModificationEvent event) {
        ...
        Stream.of(ModEntities.HORSE_GENETIC.orElse(null)).filter(DefaultAttributes::hasSupplier).map(entityType -> (EntityType<? extends LivingEntity>) entityType).toList().forEach(entity -> event.add(entity, WEIGHT.get()));
    }
}